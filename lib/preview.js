...
function attachPreview(createPreview, opts = {}, files = {}, filePath = '') {
    ...
    if (shouldPreview && Buffer.isBuffer(fileData.contents)) {
        ...
        if (opts.characters && opts.characters.trim) {
...
function choosePreviewFunction(opts = {}) {
    ...
    if (opts.words > 0) {
        Debug('chose a word preview');
        previewFunction = createWordPreview.bind(
            null,
            opts.words,
            opts.strip
        );
    } else if (opts.characters > 0 || opts.characters.count > 0) {
        Debug('chose a character preview');
        previewFunction = createCharacterPreview.bind(
            null,
            (opts.characters.count || opts.characters) - opts.continueIndicator.length,
            opts.strip
        );
    } else if (hasAnyMarkers) {
        Debug('chose a marker preview');
...
function createCharacterPreview(charCount, strip = '', fileData) {
    if (!Number.isInteger(charCount) || charCount < 1) {
        const msg = 'Generating a character count preview requires a positive integer.';
        Debug('failed with "%s"', msg);
...
function createWordPreview(wordCount, strip = '', fileData) {
    if (!Number.isInteger(wordCount) || wordCount < 1) {
        const msg = 'Generating a word count preview requires a positive integer.';
        Debug('failed with "%s"', msg);
        throw new TypeError(msg);
    }
